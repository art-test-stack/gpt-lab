from gpt_lib.utils.common import get_banner, print0, run_command
...
from gpt_lib.utils.common import run_command
...
def generate_header():
    ...
    header = f"""# gpt-lib training report
...
class Report:
    ...
    def generate(self):
        ...
        print0(f"Generating report to {report_file}")
        ...
        with open(report_file, "w", encoding="utf-8") as out_file:
            ...
            if os.path.exists(header_file):
                ...
                print0(f"Warning: {header_file} does not exist. Did you forget to run `gptlib reset`?")
            # process all the individual sections
            for file_name in EXPECTED_FILES:
                section_file = os.path.join(report_dir, file_name)
                if not os.path.exists(section_file):
                    print0(f"Warning: {section_file} does not exist, skipping")
                    continue
        ...
        print0(f"Copying report.md to current directory for convenience")
    ...
    def reset(self):
        ...
        print0(f"Reset report and wrote header to {header_file}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate or reset training reports.")
